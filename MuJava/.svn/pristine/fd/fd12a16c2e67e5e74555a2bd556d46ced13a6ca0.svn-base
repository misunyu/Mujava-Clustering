 ...
 * @author Michiaki Tatsubori
 * @version 1.0
 * @since $Id: OJSystem.java,v 1.1 2009-10-30 09:05:11 swkim Exp $
 * @see java.lang.Object
...
public class OJSystem {
	...
	/**
	 * modified by swkim to integrate with mujava
	 * 
	 */
	static ClassLoader classLoader = null;

	public static final void initialize() {
		// clear classes generated when compiling
		additionalClasses.clear();

		// clear metabind table which has relations between a name and a java
		// class
		table.clear();

		// clear GlobalEnvironment's states
		env.initialize();
	}
	...
	private static final Vector<OJClass> additionalClasses = new Vector<OJClass>();
	...
	private static Hashtable<String, Class<?>> table = new Hashtable<String, Class<?>>();
	...
	public static void metabind(String clazz, String metaclazz)
		...
		try {
			...
			if (cl == null)
				...
				c = Class.forName(metaclazz, true, cl);
			/** end of modification */
	...
	private static Class<?> searchMetaclassInTable(String clazz) {
		...
		/*
		 * a name specified using "*" (all classes in a package) More specific
		 * indication has more priority. e.g. "java.lang.*" is stronger than
		 * "java.*"
		 */{
			Class<?> result = (Class<?>) table
					.get(toPackageSuffix(clazz) + "*");
			if (result != null)
				return result;
		}
		/*
		 * a name specified using "-" (all subpackages) More specific indication
		 * has more priority. e.g. "java.lang.-" is stronger than "java.-"
		 */{
			Class<?> result = (Class<?>) table
					.get(toPackageSuffix(clazz) + "-");
			if (result != null)
		...
		for (String pack = toPackage(clazz); pack != null; pack = toPackage(pack)) {
