...
public class LOIChangePointSeeker extends AbstractChangePointSeeker {
	public LOIChangePointSeeker(ICompilationUnit sourceJavaElement,
			FileEnvironment file_env) {
		super(file_env, sourceJavaElement, "LOI");
	}
		...
		if (!isStatement(p)) {
			...
			if (retType == null) {
				MuJavaLogger.getLogger().error(
						"MethodCall_Error in LOI Mutator: "
								+ p.toFlattenString());
				return;
