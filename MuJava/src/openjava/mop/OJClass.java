...
/**
 * The <code>OJClass</code> class represents a class metaobject. If the class
 * has its valid .class file in CLASSPATH, the metaobject can behave like
 * <code>java.lang.Class</code>. If the class has its valid .oj file in the
 * packages where some classes are being compiled by OpenJava system, or in the
 * packages explicitly specified by something like OpenJava compiler option or
 * environment variable, the metaobject can have information about source code
 * just as stamements, expressions, and etc.
 * <p>
 ...
 * @author Michiaki Tatsubori
 * @version 1.0
 * @since $Id: OJClass.java,v 1.1 2009-10-30 09:05:11 swkim Exp $
 * @see java.lang.Class
...
public class OJClass implements OJMember {
	 ...
	 * <p>
	 * This constructor will be invoked by the OpenJava system. In inheriting
	 * this class, you must call this consturctor from a constructor with the
	 * same signature in your class.
	 * <p>
	 * For example, in defining a subclass <code>YourClass</code> derived from
	 * this class, at least you have to write as follows:
	 * 
	 * <pre>
	 * public YourClass(Environment outer_env, OJClass declarer, ClassDeclaration ptree) {
	 * 	super(outer_env, declarer, ptree);
	 * }
	 * </pre>
	 * 
	 * @param outer_env
	 *            environment of this class metaobject
	 * @param declarer
	 *            the class metaobject declaring this class metaobject.
	 * @param ptree
	 *            the parse tree representing this class metaobject.
	 */
	 ...
	 * <p>
	 * This constructor will be invoked only by the OpenJava system. In
	 * inheriting this class, you must call this consturctor from a constructor
	 * with the same signature in your class.
	 * <p>
	 * For example, in defining a subclass <code>YourClass</code> derived from
	 * this class, at least you have to write as follows:
	 * 
	...
	/**
	 * For dummy type of null object. This should be called only once by
	...
	/**
	 * Returns the <code>OJClass</code> object associated with the class with
	 * the given string name. Given the fully-qualified name for a class or
	 * interface, this method attempts to locate, load and link the class. If it
	 * succeeds, returns the Class object representing the class. If it fails,
	 * the method throws a OJClassNotFoundException.
	 * <p>
	 ...
	 * @return the <code>OJClass</code> descriptor for the class with the
	 *         specified name.
	 * @exception OJClassNotFoundException
	...
	/**
	 * swkim modified this method to look up the bytecode corresponding to the
	 * given class name at the OJSystem's classloader instead of System's
	 * classloader
	 * 
	 * @param name
	 * @return
	...
	private static final OJClass lookupFromByteCode(String name) {
		...
		try {
			...
			if (cl == null)
				...
				// swkim don't know the meaning of initialize loaded class, so
				// he set the value to false
				// javaClass = cl.loadClass(name);
				javaClass = Class.forName(name, false, cl);
			/** end of modification */
	 ...
	 * object.
	 * <p>
	 * This method returns the same <code>OJClass</code> object whenever it is
	 * invoked for the same <code>Class</code>object. It gurantees one-to-one
	 * correspondence between <code>Class</code> and <code>OJClass</code>.
	 */
	...
	private static final OJClass lookupFromMetaInfo(Class<?> javaClass) {
		...
		try {
			...
			if (cl == null)
				...
				metaclazz = Class.forName(mcname, true, cl);
			/** end of modification */
			...
			System.err.println("metaclass " + mcname + " for " + javaClass
					+ " not found." + " substituted by default metaclass.");
	 ...
	 * object. The generated <code>OJClass</code> object is to be registered
	...
	public static OJClass forParseTree(Environment env, OJClass declaringClass,
			ClassDeclaration ptree) throws AmbiguousClassesException,
		...
		OJClass result;
		try {
			Constructor<?> constr = metaclazz.getConstructor(new Class[] {
	...
	/**
	 * Generates a expression parse tree from a given <code>String</code>
	 * object under the given environment.
	 * 
	 * @param env
	 *            an environment for the source code.
	 * @param str
	 *            a fragment of source code representing an expression.
	 * @return an expression parse tree
	 */
	protected static final Expression makeExpression(Environment env, String str)
			throws MOPException {
		return PartialParser.makeExpression(env, str);
	}

	/**
	 * Generates an expression parse tree from a given <code>String</code>
	 * object under the environment of this class object.
	 * 
	 * @param str
	 *            a fragment of source code representing an expression.
	 * @return an expression parse tree
	...
	/**
	 * Generates a statement parse tree from a given <code>String</code>
	 * object under the given environment.
	 * 
	 * @param env
	 *            an environment for the source code.
	...
	/**
	 * Generates a statement list parse tree from a given <code>String</code>
	 * object under the given environment.
	 * 
	 * @param env
	 *            an environment for the source code.
	...
	protected static final StatementList makeStatementList(Environment env,
			String str) throws MOPException {
	 ...
	 * <code>getName</code>. If this <code>OJClass</code> object represents
	 * a primitive type, this method returns the name of the primitive type. If
	 * this <code>OJClass</code> object represents void this method returns
	 * "void".
	 * 
	...
	/**
	 * Obtains an environment of this class object. This environment has
	 * information about members and outr environments such as packages but not
	 * local information like local variable in methods of this class.
	 * 
	 ...
	 * <code>OJClass</code> object is either the same as, or is a superclass
	 * or superinterface of, the class or interface represented by the specified
	 * <code>OJClass</code> parameter. It returns <code>true</code> if so;
	 * otherwise it returns <code>false</code>. If this <code>OJClass</code>
	 * object represents a primitive type, this method returns <code>true</code>
	 * if the type represented by this <code>OJClass</code> object can accept
	 * the type represented by the specified <code>OJClass</code> parameter;
	 * otherwise it returns <code>false</code>.
	 * 
	 * <p>
	 * Specifically, this method tests whether the type represented by the
	 * specified <code>OJClass</code> parameter can be converted to the type
	 * represented by this <code>OJClass</code> object via an identity
	 * conversion or via a widening reference/primitive conversion.
	 * <p>
	 * The behavior about primitive types is different from the method of the
	 * same name of <code>java.lang.Class</code>.
	 * 
	...
	public boolean isAssignableFrom(OJClass clazz) {
		...
		// CHANGED modified by swkim, in case of trying to compare byte
		// substance and source substance
		if (clazz.toString().equals(this.toString())) {
			return true;
		}
		...
		if (this.isPrimitive()) {
			...
			if (!clazz.isPrimitive()) {
				...
				if (this == OJSystem.FLOAT
						&& "java.lang.Float".equals(clazz.getName())) {
					return true;
				}
		...
		/*
		 * getInterfaces() returns only the declared intefaces So the interfaces
		 * of the superclasses should be checked.
		 */
		if (this.isInterface()) {
	...
	/**
	 * Determines if the specified <code>OJClass</code> object represents an
	 * interface type.
	 * 
	...
	/**
	 * Determines if this <code>OJClass</code> object represents an array
	 * class.
	 * 
	 ...
	 * 
	 * <p>
	 * There are nine predefined <code>OJClass</code> objects to represent the
	 ...
	 * 
	 * <p>
	 * These objects may be accessed via the following public static final
	 * variables, and are the only <code>OJClass</code> objects for which this
	 ...
	 * 
	 * @return <code>true</code> if this object represents a wrapper class for
	 *         a primitive type; <code>false</code> otherwise.
	 */
	 ...
	 * 
	 * @return The wrapper class for this primitive type. <code>null</code>
	 *         for void.
	 */
	...
	/**
	 * Obtains the real type class if this class represents a primitive wrapper
	 * type.
	 * <p>
	 ...
	 * class, primitive type, or void) represented by this <code>OJClass</code>
	 * object, as a <code>String</code>.
	 * 
	 * <p>
	 * If this <code>OJClass</code> object represents a class of arrays, then
	 * the internal form of the name consists of the name of the element type in
	 * Java signature format, followed by one or more "<tt>[]</tt>"
	 * characters representing the depth of array nesting. This representation
	 * differs from that of <code>java.lang.Class.forName()</code>. Thus:
	 * 
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 * 
	 * @return the simple name of the class or interface represented by this
	 *         object.
	 */
	...
	/**
	 * Gets the package name for this class as a <code>String</code>. Null is
	 * returned if its package was not specified in source code of this class.
	 * 
	 * @return the package name of the class, or null if its package was not
	...
	/**
	 * Determines if the specified class object is in the same package as this
	 * class object.
	 * 
	 * <p>
	...
	/**
	 * Returns the <code>OJClass</code> representing the superclass of the
	 * entity (class, interface, primitive type or void) represented by this
	 * <code>OJClass</code>. If this <code>OJClass</code> represents either
	 * the <code>java.lang.Object</code> class, an interface, a primitive
	 * type, or void, then null is returned. If this object represents an array
	 * class then the <code>OJClass</code> object representing the
	 * <code>java.lang.Object</code> class is returned.
	 * 
	 ...
	 * </pre>
	 * 
	 * </blockquote> suppose the value of <code>clazz</code> is an class
	 * object for the class <code>Shimmer</code>; the value of the
	 ...
	 * </pre>
	 * 
	 * </blockquote> is the <code>OJClass</code> object that represents
	 * interface <code>FloorWax</code>; and the value of: <blockquote>
	 * 
	 * <pre>
	 ...
	 * 
	 * </blockquote> is the <code>OJClass</code> object that represents
	 * interface <code>DessertTopping</code>.
	 * 
	 * <p>
	...
	/**
	 * Returns the Java language modifiers and the user defined modifiers for
	 * this class or interface, as a <code>OJModifier</code> object.
	 * 
	 * <p>
	 * If the underlying class is an array class, then its <code>public</code>,
	 * <code>private</code> and <code>protected</code> modifiers are the
	 * same as those of its component type. If this <code>OJClass</code>
	 * represents a primitive type or void, its <code>public</code> modifier
	 * is always <code>true</code>, and its <code>protected</code> and
	 * <code>private</code> modifers are always <code>false</code>. If this
	 * object represents an array class, a primitive type or void, then its
	 * <code>final</code> modifier is always <code>true</code> and its
	 * interface modifer is always <code>false</code>. The values of its
	 * other modifiers are not determined by this specification.
	 * 
	...
	/**
	 * Obtains an parse tree of suffix in extended syntax starting with the
	 * specified keyword. Returned <code>openjava.ptree.ParseTree</code>
	 * object has a structure built by an
	 * <code>openjava.syntax.SyntaxRule</code> object returned via the method
	 * <code>getDeclSuffixRule(String)</code>.
	 * 
	...
	/**
	 * If the class or interface represented by this <code>OJClass</code>
	 * object is a member of another class, returns the <code>OJClass</code>
	 * object representing the class in which it was declared. This method
	 * returns null if this class or interface is not a member of any other
	 * class. If this <code>OJClass</code> object represents an array class, a
	...
	private static final OJConstructor[] removeThePrivates(
	...
	private static final OJConstructor[] removeTheDefaults(
			OJConstructor[] src_constrs) {
	...
	private static final OJConstructor[] removeTheNonPublics(
			OJConstructor[] src_constrs) {
	...
	private static final OJConstructor pickupConstructor(
			OJConstructor[] src_constrs, OJClass[] parameterTypes) {
		return Toolbox.pickupConstructor(src_constrs, parameterTypes);
	}

	private static final OJMethod pickupAcceptableMethod(
			OJMethod[] src_methods, String name, OJClass[] parameterTypes) {
		return Toolbox
				.pickupAcceptableMethod(src_methods, name, parameterTypes);
	}
	...
	/**
	 * Returns an array containing <code>OJClass</code> objects representing
	 * all the <em>public</em> classes and interfaces that are members of the
	 * class represented by this <code>OJClass</code> object. This includes
	 * public class and interface members inherited from superclasses and public
	 * class and interface members declared by the class. This method returns an
	 * array of length 0 if this <code>OJClass</code> object has no public
	 * member classes or interfaces. This method also returns an array of length
	 * 0 if this <code>OJClass</code> object represents a primitive type, an
	 * array class, or void.
	 * 
	 */
	...
	/**
	 * Returns an array containing <code>OJField</code> objects reflecting all
	 * the accessible <em>public</em> fields of the class or interface
	 * represented by this <code>OJClass</code> object. The elements in the
	 * array returned are not sorted and are not in any particular order. This
	 * method returns an array of length 0 if the class or interface has no
	 * accessible public fields, or if it represents an array class, a primitive
	 * type, or void.
	 * 
	 * <p>
	 * Specifically, if this <code>OJClass</code> object represents a class,
	 * this method returns the public fields of this class and of all its
	 * superclasses. If this <code>OJClass</code> object represents an
	...
	/**
	 * Returns an array containing <code>OJMethod</code> objects reflecting
	 * all the <em>public</em> member methods of the class or interface
	 * represented by this <code>OJClass</code> object, including those
	 * declared by the class or interface and and those inherited from
	 * superclasses and superinterfaces. The elements in the array returned are
	 * not sorted and are not in any particular order. This method returns an
	 * array of length 0 if this <code>OJClass</code> object represents a
	 * class or interface that has no public member methods, or if this
	 * <code>OJClass</code> object represents an array class, primitive type,
	 * or void.
	 * 
	 * @see openjava.mop.OJMethod
	 */
	 ...
	 * represented by this <code>OJClass</code> object. An array of length 0
	 * is returned if the class has no public constructors, or if the class is
	 * an array class, or if the class reflects a primitive type or void.
	 * 
	 * @see openjava.mop.OJConstructor
	 */
	 ...
	 * <em>public</em> member field of the class or interface represented by
	 * this <code>OJClass</code> object. The <code>name</code> parameter is
	 * a <code>String</code> specifying the simple name of the desired field.
	 * 
	 * @exception NoSuchMemberException
	...
	/**
	 * Returns a <code>OJMethod</code> object that reflects the specified
	 * public member method of the class or interface represented by this
	 ...
	 * parameter types, in declared order. If <code>parameterTypes</code> is
	 * <code>null</code>, it is treated as if it were an empty array.
	 * 
	 * @exception NoSuchMemberException
	 *                if a matching method is not found or if then name is
	 *                "&lt;init>"or "&lt;clinit>".
	 * @see openjava.mop.OJMethod
	 */
	...
	/**
	 * Returns a <code>OJConstructor</code> object that reflects the specified
	 * public constructor of the class represented by this <code>OJClass</code>
	 * object. The <code>parameterTypes</code> parameter is an array of
	 * <code>OJClass</code> objects that identify the constructor's formal
	 * parameter types, in declared order.
	 * 
	 * <p>
	 * The constructor to reflect is the public constructor of the class
	 * represented by this <code>OJClass</code> object whose formal parameter
	...
	/**
	 * Returns an array containing <code>OJClass</code> objects representing
	 * all the classes and interfaces which are members of the class represented
	 * by this <code>OJClass</code> object, accessible from the situation
	 * represented by the given <code>OJClass</code> object. This includes
	 * class and interface members inherited from superclasses and declared
	 * class and interface members accessible from the given situation. This
	 * method returns an array of length 0 if this <code>OJClass</code> object
	 * has no public member classes or interfaces. This method also returns an
	 * array of length 0 if this <code>OJClass</code> object represents a
	 * primitive type, an array class, or void.
	 * 
	 * <p>
	 * The accessiblity depends on the package of the class, modifiers of each
	 * members, and the package of the situation.
	 * 
	 */
	...
	/**
	 * Returns an array containing <code>OJField</code> objects reflecting all
	 * the fields of the class or interface represented by this
	 * <code>OJClass</code> object, accessible from the situation represented
	 * by the given <code>OJClass</code> object. The elements in the array
	 * returned are not sorted and are not in any particular order. This method
	 * returns an array of length 0 if the class or interface has no accessible
	 * public fields, or if it represents an array class, a primitive type, or
	 * void.
	 * 
	 * <p>
	 * Specifically, if this <code>OJClass</code> object represents a class,
	...
	/**
	 * Returns an array containing <code>OJMethod</code> objects reflecting
	 * all the member methods of the class or interface represented by this
	 * <code>OJClass</code> object, accesible from the situation represented
	 * by the given <code>OJClass</code> object. Returned methods include
	...
	/**
	 * Returns an array containing <code>OJConstructor</code> objects
	 * reflecting all the constructors of the class represented by this
	 * <code>OJClass</code> object, accesible from the situation represented
	 * by the given <code>OJClass</code> object. An array of length 0 is
	 * returned if the class has no public constructors, or if the class is an
	...
	/**
	 * Returns a <code>OJField</code> object that reflects the specified
	 * member field accesible from the situation represented by the given
	 * <code>OJClass</code> object. The <code>name</code> parameter is a
	...
	/**
	 * Returns a <code>OJMethod</code> object that reflects the specified
	 * member method accesible from the situation represented by the given
	 * <code>OJClass</code> object. The <code>name</code> parameter is a
	 * <code>String</code> specifying the simple name the desired method. The
	 * <code>parameterTypes</code> parameter is an array of
	 * <code>OJClass</code> objects that identify the method's formal
	 ...
	 */
	public OJMethod getMethod(String name, OJClass[] parameterTypes,
			OJClass situation) throws NoSuchMemberException {
		OJMethod method = pickupMethod(getMethods(situation), name,
				parameterTypes);
		if (method != null)
			return method;
		Signature sign = new Signature(name, parameterTypes);
	...
	/**
	 * Returns a <code>OJConstructor</code> object that reflects the specified
	 * constructor accesible from the situation represented by the given
	 * <code>OJClass</code> object. The <code>parameterTypes</code>
	 * parameter is an array of <code>OJClass</code> objects that identify the
	 * constructor's formal parameter types, in declared order.
	 * 
	 * <p>
	 * The constructor to reflect is the constructor of the class represented by
	 * this <code>OJClass</code> object whose formal parameter types match
	 * those specified by <code>parameterTypes</code>.
	 * 
	 * @exception NoSuchMemberException
	 *                if a matching method is not found.
	 * @see openjava.mop.OJConstructor
	...
	public OJConstructor getConstructor(OJClass[] parameterTypes,
			OJClass situation) throws NoSuchMemberException {
		OJConstructor constr = pickupConstructor(getConstructors(situation),
				parameterTypes);
		if (constr != null)
			return constr;
		Signature sign = new Signature(parameterTypes);
	...
	public OJMethod getAcceptableMethod(String name, OJClass[] parameterTypes,
			OJClass situation) throws NoSuchMemberException {
		OJMethod method = pickupAcceptableMethod(getMethods(situation), name,
				parameterTypes);
		if (method != null)
			return method;
		Signature sign = new Signature(name, parameterTypes);
	...
	/**
	 * Returns an array of <code>OJClass</code> objects reflecting all the
	 * classes and interfaces declared as members of the class represented by
	 * this <code>OJClass</code> object. This includes public, protected,
	 * default (package) access, and private classes and interfaces declared by
	 * the class, but excludes inherited classes and interfaces. This method
	 * returns an array of length 0 if the class declares no classes or
	 * interfaces as members, or if this <code>OJClass</code> object
	 * represents a primitive type, an array class, or void.
	 * 
	 * <p>
	 * This method may be overriden to provide proper information in the
	 * extended language.
	 */
	...
	/**
	 * Returns an array of <code>OJField</code> objects reflecting all the
	 * fields declared by the class or interface represented by this
	 * <code>OJClass</code> object. This includes public, protected, default
	 * (package) access, and private fields, but excludes inherited fields. The
	 * elements in the array returned are not sorted and are not in any
	 * particular order. This method returns an array of length 0 if the class
	 * or interface declares no fields, or if this <code>OJClass</code> object
	 * represents a primitive type, an array class, or void.
	 * 
	 * <p>
	 * This method may be overriden to provide proper information in the
	...
	/**
	 * Returns an array of <code>OJMethod</code> objects reflecting all the
	 * methods declared by the class or interface represented by this
	 * <code>OJClass</code> object. This includes public, protected, default
	 * (package) access, and private methods, but excludes inherited methods.
	 * The elements in the array returned are not sorted and are not in any
	 * particular order. This method returns an array of length 0 if the class
	 * or interface declares no methods, or if this <code>OJClass</code>
	 * object represents a primitive type, an array class, or void. The class
	 * initialization method <code>&lt;clinit&gt;</code> is not included in
	 * the returned array. If the class declares multiple public member methods
	...
	/**
	 * Returns an array of <code>OJConstructor</code> objects reflecting all
	 * the constructors declared by the class represented by this
	 * <code>OJClass</code> object. These are public, protected, default
	 * (package) access, and private constructors. The elements in the array
	 * returned are not sorted and are not in any particular order. If the class
	 * has a default constructor, it is included in the returned array. This
	 * method returns an array of length 0 if this <code>OJClass</code> object
	 * represents an interface, a primitive type, an array class, or void.
	 * 
	 * <p>
	 * This method may be overriden to provide proper information in the
	 * extended language.
	 ...
	 * field. Note that this method will reflect the <code>length</code> field
	 * of an array class.
	 * 
	 * @exception NoSuchMemberException
	 *                if a field with the specified name is not found.
	 ...
	 * <code>OJClass</code> objects that identify the method's formal
	 * parameter types, in declared order. If more than one method with the same
	...
	public final OJMethod getDeclaredMethod(String name,
			OJClass[] parameterTypes) throws NoSuchMemberException {
		OJMethod method = pickupMethod(getDeclaredMethods(), name,
				parameterTypes);
		if (method != null)
			return method;
		Signature sign = new Signature(name, parameterTypes);
	...
	public final OJConstructor getDeclaredConstructor(OJClass[] parameterTypes)
			throws NoSuchMemberException {
		OJConstructor constr = pickupConstructor(getDeclaredConstructors(),
				parameterTypes);
		if (constr != null)
			return constr;
		Signature sign = new Signature(parameterTypes);
	...
	public OJClass makeCopy(String qname) throws MOPException {
		DebugOut.println("makeCopy() of " + getName() + " with a new name: "
				+ qname);
		try {
	...
	public final void waitTranslation(OJClass clazz) throws MOPException {
		...
		synchronized (OJSystem.waitingPool) {
			if (OJSystem.waitingPool.contains(clazz)) {
				System.err.println("a dead lock detected between " + getName()
						+ " and " + clazz.getName());
				return;
			}
	...
	/**
	 * Under implementation. Modifiers of members should be considered
	 * carefully.
	 * 
	...
	public ClassDeclaration translateDefinition(Environment env,
			ClassDeclaration decl) throws MOPException {
		OJClass base = getSuperclass();
		if (base != null)
	...
	public OJField resolveException(NoSuchMemberException e, String name)
			throws NoSuchMemberException {
		System.err.println("no such " + new Signature(name) + " in "
				+ toString());
		throw e;
	}

	public OJMethod resolveException(NoSuchMemberException e, String name,
			OJClass[] argtypes) throws NoSuchMemberException {
		System.err.println("no such " + new Signature(name, argtypes) + " in "
				+ toString());
		throw e;
	}
	...
	public final Enumeration<String> getMetaInfoKeys() {
...
/**
 * The abstract class <code>OJClassImp</code> provides an interface to an
 * implementation of OJClass.
 */
abstract class OJClassImp {
	...
	abstract Enumeration<String> getMetaInfoKeys();
...
class OJClassByteCode extends OJClassImp {
	...
	Enumeration<String> getMetaInfoKeys() {
		return metainfo.keys();
	}

	Enumeration<String> getMetaInfoElements() {
...
class OJClassSourceCode extends OJClassImp {
	...
	private Vector<OJClass> classes = new Vector<OJClass>();

	private Vector<OJField> fields = new Vector<OJField>();

	private Vector<OJMethod> methods = new Vector<OJMethod>();

	private Vector<OJConstructor> constrs = new Vector<OJConstructor>();

	private MetaInfo metainfo;

	/* -- constructors -- */

	OJClassSourceCode(OJClass holder, Environment outer_env, OJClass declarer,
			ClassDeclaration ptree) {
	...
	OJClass getSuperclass() {
		if (isInterface()) {
			...
			String basename = (base == null) ? "java.lang.Object" : base
					.toString();
	...
	InputStream getResourceAsStream(String name) throws CannotInspectException {
