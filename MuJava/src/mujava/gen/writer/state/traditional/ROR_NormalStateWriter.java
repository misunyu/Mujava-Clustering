...
public class ROR_NormalStateWriter extends TraditionalMutantNormalStateWriter {
	public static TraditionalMutantNormalStateWriter getMutantCodeWriter(
			RORChangePointSeeker mutator) throws IOException {
		ROR_NormalStateWriter mutantWriter = new ROR_NormalStateWriter(mutator
				.getFileEnvironment());
	...
	public ROR_NormalStateWriter(Environment env) throws IOException {
		super(env, "ROR", false);
...
//		System.out.println("visit(BinaryExpression p) START");
		...
		if (super.currentChangePoint.isSamePoint(p, currentMethod)) {
			...
			if(clz == OJSystem.BOOLEAN){
				newOperator = ROR.getROROperatorForBoolean(operator, extractMutantOpArray(mutantID, generatedMutantID)) ;
			}else if (clz.isPrimitive()){
				newOperator = ROR.getNonRedundantROROp(operator, extractMutantOpArray(mutantID, generatedMutantID)) ;
			}else{
				newOperator = ROR.getROROperatorForObject(operator, extractMutantOpArray(mutantID, generatedMutantID)) ;
			}
			...
			if(newOperator==ROR.TRUE){
					...
					out.print("/**here**/ " + getRORCode(newOperator) + " ");
			...
			*/
			...
			m.setChangeLog(removeNewline(p.toString() + " => "
						+ ROR.getMutatedExp(leftExp.toFlattenString(),rightExp.toFlattenString(),newOperator)));
