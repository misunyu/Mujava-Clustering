...
public class AOIU_MSGWriter extends TraditionalMutantMSGWriter {
	public static MutantWriter getMutantCodeWriter(AOIUChangePointSeeker mutator)
			throws IOException {
		AOIU_MSGWriter mutantWriter = new AOIU_MSGWriter(mutator
				.getFileEnvironment());
	...
	public AOIU_MSGWriter(Environment env) throws IOException {
		super(env, "AOIU");
	}
	...
	public void visit(MethodCall mc) throws ParseTreeException {
		if (phase == IMPLEMENTED) {
			...
			if (cPoint != null) {
				...
				String log_str = mc.toString() + " => AOIDU()";
				out.print("( MSG.MutantMonitor.ExecuteChangePoint["
						+ cPoint.getID() + "] ? -(");
				...
				m.setMutationOperatorName(this.getMutationOperatorName());
				m.setChangeLog(removeNewline(log_str));
				m.setMutantID(mutantID + "_" + UnaryExpression.MINUS);
