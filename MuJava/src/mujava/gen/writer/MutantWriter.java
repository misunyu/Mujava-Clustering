...
public class MutantWriter extends GenericCodeWriter {
	...
	protected ArrayList<Integer> extractMutantOpArray(String mutantPrefix, List<String> generatedMutantID){
		ArrayList<Integer> myArray = new ArrayList<Integer>();
		String mutantID;
		String moIndex;
		for(int i=0;i<generatedMutantID.size();i++){
			mutantID = generatedMutantID.get(i);
			if(mutantID.startsWith(mutantPrefix)){
				moIndex = mutantID.substring(mutantPrefix.length()+1);
				myArray.add(new Integer(moIndex));				
			}
		}	
		return myArray;
	}
