...
public class ASRSNuJavaWriter extends TraditionalMutantNuJavaWriter {
	public static MutantWriter getMutantCodeWriter(ASRSChangePointSeeker mutator)
			throws IOException {
		ASRSNuJavaWriter mutantWriter = new ASRSNuJavaWriter(mutator
				.getFileEnvironment());
	...
	public ASRSNuJavaWriter(Environment env) throws IOException {
		super(env, "ASRS");
	}
		...
		if (phase == IMPLEMENTED) {
			...
			if (cPoint != null) {
				...
				out.print("nujava.MutantMonitor.ExecuteChangePoint["
						+ cPoint.getID() + "] ? ");
				out.print("(" + retType.toString()
						+ ")nujava.traditional.ASRSMetaMutant.ASRSGen(");
