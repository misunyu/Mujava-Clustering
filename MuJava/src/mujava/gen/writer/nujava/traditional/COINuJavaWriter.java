...
public class COINuJavaWriter extends TraditionalMutantNuJavaWriter {
	public static MutantWriter getMutantCodeWriter(COIChangePointSeeker mutator)
			throws IOException {
		COINuJavaWriter mutantWriter = new COINuJavaWriter(mutator
				.getFileEnvironment());
	...
	public COINuJavaWriter(Environment env) throws IOException {
		super(env, "COI");
	}
	...
	private boolean writeCode(Expression p) throws ParseTreeException {
		if (phase == IMPLEMENTED) {
			...
			if (cPoint != null) {
				...
				String log_str = p.toString() + " => COI()";
				out.print("(nujava.MutantMonitor.ExecuteChangePoint[");
				out.print(cPoint.getID());
				out.print("] ? nujava.traditional.COIDMetaMutant.COIDGen(");
				phase = ORIGINAL;
				super.visit(p);
	...
	public void visit(MethodCall mc) throws ParseTreeException {
		if (phase == IMPLEMENTED) {
			...
			if (cPoint != null) {
				...
				String log_str = mc.toString() + " => COI()";
				out.print("(nujava.MutantMonitor.ExecuteChangePoint[");
				out.print(cPoint.getID());
				out.print("] ? nujava.traditional.COIDMetaMutant.COIDGen(");
				phase = ORIGINAL;
